...
export function GraphTab() {
  const { issues, themes, stateOf } = useDepFlow()
  ...
  const nw = 104
  const nh = 46
  const themeColor = (k: string) => themes.find((t) => t.key === k)?.color ?? '#6e7bff'
          ...
          {nodes.map(({ it, x, y }) => {
            ...
            return (
              <g key={it.id} opacity={op} style={{ cursor: 'pointer' }} onClick={() => openIssue(it.id)}>
                ...
                <text x={x + 12} y={y + 17} fontSize="8.5" fontWeight="700" fill="#5d6173" fontFamily="var(--mono)">
                  {it.id}
                </text>
