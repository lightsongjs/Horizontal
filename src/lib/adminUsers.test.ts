...
import { listUsers, createUser, setAccess, setName, resetPassword, deleteUser } from './adminUsers'

beforeEach(() => invoke.mockReset())

describe('listUsers', () => {
  it('invokes admin-users with list_users and returns users', async () => {
    invoke.mockResolvedValue({ data: { users: [{ id: '1', email: 'a@b.c', name: 'Ana', access: [] }] }, error: null })
    const users = await listUsers()
    expect(invoke).toHaveBeenCalledWith('admin-users', { body: { action: 'list_users' } })
    expect(users).toEqual([{ id: '1', email: 'a@b.c', name: 'Ana', access: [] }])
  })
...
describe('createUser', () => {
  it('sends create_user with email, password, name, access', async () => {
    invoke.mockResolvedValue({ data: { id: 'new' }, error: null })
    const id = await createUser('a@b.c', 'pw', 'Ana', [{ project_id: 'p', role: 'write' }])
    expect(invoke).toHaveBeenCalledWith('admin-users', {
      body: { action: 'create_user', payload: { email: 'a@b.c', password: 'pw', name: 'Ana', access: [{ project_id: 'p', role: 'write' }] } },
    })
    expect(id).toBe('new')
  })

  // Numele e optional: fara el nu se scrie niciun rand in `assignees`, iar
  // interfata cade pe partea locala a emailului (`memberDisplayName`). Un `''`
  // trimis la server ar fi o valoare, nu o absenta — de-aia devine `null`.
  it('trimite null cand numele e gol sau doar spatii', async () => {
    invoke.mockResolvedValue({ data: { id: 'new' }, error: null })
    await createUser('a@b.c', 'pw', '   ', [])
    expect(invoke).toHaveBeenCalledWith('admin-users', {
      body: { action: 'create_user', payload: { email: 'a@b.c', password: 'pw', name: null, access: [] } },
    })
  })
})

describe('setName', () => {
  it('trimite set_name cu user_id si numele curatat', async () => {
    invoke.mockResolvedValue({ data: { ok: true }, error: null })
    await setName('u1', '  Mihai  ')
    expect(invoke).toHaveBeenCalledWith('admin-users', {
      body: { action: 'set_name', payload: { user_id: 'u1', name: 'Mihai' } },
    })
  })
})
