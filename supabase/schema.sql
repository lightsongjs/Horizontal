-- DepFlow schema. Run once in the Supabase SQL editor.
...
-- Simple issues (no type/epic). Waves and themes per-project. No seed data.
...
create table if not exists issues (id text primary key, project_id text not null references projects(id) on delete cascade, title text not null, details text, theme text, wave int not null default 1, done boolean not null default false);
